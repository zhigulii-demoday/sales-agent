...
from api.models import companies_post
...
async def get_quiz_id(
    ...
    answer = await session.get(Company, id)
...
@router.post("",
            ...
            tags=[tag])
async def post_quiz(
    companies_post: companies_post,
    session: AsyncSession = Depends(get_session)
):
    
    new = Company(
        name = companies_post.name,
        description = companies_post.description
    )
    session.add(new)
